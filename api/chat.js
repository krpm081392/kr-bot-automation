export default async function handler(req, res) {
  ...
  if (!apiKey) return res.status(200).json({ reply: "Gemini is not connected yet. Add GEMINI_API_KEY in Vercel Environment Variables, then redeploy." });
  const { message } = req.body || {};
  if (!message) return res.status(400).json({ reply: "Please type a question." });

  const prompt = `You are KR Worker Bot, the sales assistant for KR Bot Automation.
Pricing:
Messenger Bot: $20.
Messenger + WhatsApp: $100.
Website + Messenger Bot: $499.
Contact: WhatsApp +385 99 219 4687, email krbotautomation@gmail.com.
Answer only about bot automation, WhatsApp, Messenger, lead capture, website packages, and pricing. Keep replies short and friendly.
